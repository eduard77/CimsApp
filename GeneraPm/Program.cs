...
// ── Database ──────────────────────────────────────────────────────────────────
builder.Services.AddScoped<GeneraPm.Services.Tenancy.ITenantContext, GeneraPm.Services.Tenancy.HttpTenantContext>();
builder.Services.AddScoped<GeneraPm.Services.Audit.AuditInterceptor>();
builder.Services.AddDbContext<GeneraPmDbContext>((sp, o) =>
    o.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection"))
     .AddInterceptors(sp.GetRequiredService<GeneraPm.Services.Audit.AuditInterceptor>()));
builder.Services.AddScoped<IProjectProvisioningService, ProjectProvisioningService>();
builder.Services.AddScoped<GeneraPm.Services.Iso19650.Iso19650FilenameValidator>();
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(o =>
    {
        o.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            // "genera:role" (Genera PM GlobalRole) instead of the default
            ...
            // `FindFirst(ClaimTypes.Role)` will see only the genera:role
            ...
            // roles onto genera:role.
            RoleClaimType            = GeneraPm.Services.Tenancy.HttpTenantContext.GlobalRoleClaimType,
        };
        ...
        // logic lives in `GeneraPm.Services.Auth.TokenRevocation`
        // for unit testing.
        o.Events = new Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerEvents
        {
            ...
            OnTokenValidated = async ctx =>
            {
                ...
                var db = ctx.HttpContext.RequestServices.GetRequiredService<GeneraPmDbContext>();
                var user = await db.Users.IgnoreQueryFilters()
                    .FirstOrDefaultAsync(u => u.Id == userId);
                var iat = (ctx.SecurityToken as System.IdentityModel.Tokens.Jwt.JwtSecurityToken)
                    ?.IssuedAt
                    ?? DateTime.MinValue;
                if (GeneraPm.Services.Auth.TokenRevocation.IsRevoked(user, iat))
                {
...
builder.Services.AddSingleton<GeneraPm.Services.Auth.ILoginAttemptTracker,
    GeneraPm.Services.Auth.LoginAttemptTracker>();
...
builder.Services.AddScoped<GeneraPm.Services.Notifications.INotificationPusher,
    GeneraPm.Services.Notifications.NotificationPusher>();
...
builder.Services.Configure<GeneraPm.Services.Email.EmailOptions>(
    builder.Configuration.GetSection("Email"));
builder.Services.AddSingleton<GeneraPm.Services.Email.EmailQueue>();
var emailEnabled = builder.Configuration.GetValue<bool>("Email:Enabled");
if (emailEnabled)
{
    builder.Services.AddScoped<GeneraPm.Services.Email.IEmailSender,
        GeneraPm.Services.Email.SmtpEmailSender>();
}
else
{
    builder.Services.AddScoped<GeneraPm.Services.Email.IEmailSender,
        GeneraPm.Services.Email.NoopEmailSender>();
}
builder.Services.AddHostedService<GeneraPm.Services.Email.EmailDispatcherHostedService>();

// T-S14-04 AlertRule + threshold evaluator.
builder.Services.AddScoped<AlertRuleService>();
builder.Services.AddHostedService<GeneraPm.Services.Alerts.ThresholdEvaluatorHostedService>();

// T-S15-02 Search & Discovery. One ISearchProvider per
// searchable entity type; SearchAggregatorService fans out
// to all of them.
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.DocumentSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.RfiSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.ActionSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.RiskSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.ChangeRequestSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.EarlyWarningSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.ISearchProvider, GeneraPm.Services.Search.CompensationEventSearchProvider>();
builder.Services.AddScoped<GeneraPm.Services.Search.SearchAggregatorService>();
...
// primitives.
builder.Services.AddScoped<GeneraPm.Services.Admin.UserAdminService>();
builder.Services.AddScoped<GeneraPm.Services.Admin.OrganisationAdminService>();
builder.Services.AddScoped<GeneraPm.Services.Admin.InvitationAdminService>();
builder.Services.AddScoped<GeneraPm.Services.Admin.AuditAdminService>();
...
builder.Services.AddScoped<GeneraPm.Services.Compliance.EvidenceLibraryService>();
builder.Services.AddScoped<GeneraPm.Services.Compliance.AuditorAdminService>();
builder.Services.AddScoped<GeneraPm.Services.Compliance.AuditExportService>();
...
builder.Services.AddScoped<GeneraPm.Services.SiteUserWorkflows.DailyDiaryService>();
builder.Services.AddScoped<GeneraPm.Services.SiteUserWorkflows.NcrService>();
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Genera PM API", Version = "v1", Description = "ISO 19650 + PMBOK 8" });
...
app.UseMiddleware<GeneraPm.Middleware.SecurityHeadersMiddleware>();

if (app.Environment.IsDevelopment())
{
    app.UseSwagger();
    app.UseSwaggerUI(c => { c.SwaggerEndpoint("/swagger/v1/swagger.json", "Genera PM API v1"); c.RoutePrefix = "swagger"; });
}
else
{
...
app.MapHub<GeneraPm.Services.Notifications.NotificationsHub>("/hubs/notifications");
app.MapRazorComponents<GeneraPm.Components.App>()
   .AddInteractiveServerRenderMode();

// ── Health check ──────────────────────────────────────────────────────────────
app.MapGet("/health", async (GeneraPmDbContext db) =>
{
...
{
    var db = scope.ServiceProvider.GetRequiredService<GeneraPmDbContext>();
