...
public class BlazorApiClient(IHttpClientFactory factory, UiStateService state)
{
    ...
    public async Task<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminUserListItem>?> AdminListUsersAsync(string? search = null, int skip = 0, int take = 50)
    {
        try { var url = $"api/v1/admin/users?skip={skip}&take={take}" + (string.IsNullOrWhiteSpace(search) ? "" : $"&search={Uri.EscapeDataString(search)}"); var b = await Http().GetStringAsync(url); return Extract<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminUserListItem>>(b, "data"); }
        catch { return null; }
    }

    public async Task<(bool Ok, string? Error)> AdminSetGlobalRoleAsync(Guid userId, GeneraPm.Models.UserRole? role)
    {
        try { var r = await Http().PutAsJsonAsync($"api/v1/admin/users/{userId}/global-role", new GeneraPm.Services.Admin.SetGlobalRoleRequest(role)); var b = await r.Content.ReadAsStringAsync(); return r.IsSuccessStatusCode ? (true, null) : (false, ExtractError(b)); }
    ...
    public async Task<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminOrgListItem>?> AdminListOrgsAsync(string? search = null, int skip = 0, int take = 50, bool includeInactive = false)
    {
        try { var url = $"api/v1/admin/organisations?skip={skip}&take={take}&includeInactive={includeInactive}" + (string.IsNullOrWhiteSpace(search) ? "" : $"&search={Uri.EscapeDataString(search)}"); var b = await Http().GetStringAsync(url); return Extract<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminOrgListItem>>(b, "data"); }
        catch { return null; }
    }

    public async Task<(bool Ok, string? Error)> AdminUpdateOrgAsync(Guid orgId, string name, string? country)
    {
        try { var r = await Http().PutAsJsonAsync($"api/v1/admin/organisations/{orgId}", new GeneraPm.Services.Admin.UpdateOrganisationRequest(name, country)); var b = await r.Content.ReadAsStringAsync(); return r.IsSuccessStatusCode ? (true, null) : (false, ExtractError(b)); }
        catch (Exception ex) { return (false, ex.Message); }
    }
    ...
    public async Task<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminInvitationListItem>?> AdminListInvitationsAsync(Guid? organisationId = null, int skip = 0, int take = 50)
    {
        try { var url = $"api/v1/admin/invitations?skip={skip}&take={take}" + (organisationId is { } id ? $"&organisationId={id}" : ""); var b = await Http().GetStringAsync(url); return Extract<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminInvitationListItem>>(b, "data"); }
        catch { return null; }
    }
    ...
    public async Task<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminAuditListItem>?> AdminListAuditAsync(DateTime? from = null, DateTime? to = null, string? action = null, string? entity = null, Guid? userId = null, int skip = 0, int take = 50)
    {
        try
        {
            ...
            return Extract<GeneraPm.Services.Admin.AdminPagedResult<GeneraPm.Services.Admin.AdminAuditListItem>>(b, "data");
        }
        catch { return null; }
    }
    ...
    public async Task<GeneraPm.Services.Compliance.EvidencePagedResult?> ListEvidenceAsync(Guid projectId, ComplianceArea? area = null, int skip = 0, int take = 50)
    {
        try { var url = $"api/v1/projects/{projectId}/evidence?skip={skip}&take={take}" + (area is { } a ? $"&area={a}" : ""); var b = await Http().GetStringAsync(url); return Extract<GeneraPm.Services.Compliance.EvidencePagedResult>(b, "data"); }
        catch { return null; }
    }

    public async Task<(bool Ok, string? Error)> AddEvidenceAsync(Guid projectId, GeneraPm.Services.Compliance.AddEvidenceRequest req)
    {
    ...
    public async Task<GeneraPm.Services.Compliance.AuditorAssignmentPagedResult?> ListAuditorAssignmentsAsync(Guid? projectId = null, bool includeRevoked = false, int skip = 0, int take = 50)
    {
        try { var url = $"api/v1/admin/auditor-assignments?skip={skip}&take={take}&includeRevoked={includeRevoked}" + (projectId is { } p ? $"&projectId={p}" : ""); var b = await Http().GetStringAsync(url); return Extract<GeneraPm.Services.Compliance.AuditorAssignmentPagedResult>(b, "data"); }
        catch { return null; }
    }

    public async Task<(bool Ok, string? Error)> CreateAuditorAssignmentAsync(GeneraPm.Services.Compliance.CreateAuditorAssignmentRequest req)
    {
    ...
    public async Task<GeneraPm.Services.SiteUserWorkflows.DailyDiaryPagedResult?> ListDiaryAsync(Guid projectId, DateOnly? from = null, DateOnly? to = null, int skip = 0, int take = 50)
    {
        try
        {
            var qs = new List<string> { $"skip={skip}", $"take={take}" };
            ...
            return Extract<GeneraPm.Services.SiteUserWorkflows.DailyDiaryPagedResult>(b, "data");
        }
        catch { return null; }
    }

    public async Task<(bool Ok, string? Error)> CreateDiaryAsync(Guid projectId, GeneraPm.Services.SiteUserWorkflows.CreateDailyDiaryRequest req)
    {
    ...
    public async Task<GeneraPm.Services.SiteUserWorkflows.NcrPagedResult?> ListNcrsAsync(Guid projectId, NcrState? status = null, int skip = 0, int take = 50)
    {
        try { var url = $"api/v1/projects/{projectId}/ncrs?skip={skip}&take={take}" + (status is { } s ? $"&status={s}" : ""); var b = await Http().GetStringAsync(url); return Extract<GeneraPm.Services.SiteUserWorkflows.NcrPagedResult>(b, "data"); }
        catch { return null; }
    }

    public async Task<(bool Ok, string? Error, string? Number)> CreateNcrAsync(Guid projectId, GeneraPm.Services.SiteUserWorkflows.CreateNcrRequest req)
    {
        try
        {
    ...
    public async Task<(bool Ok, string? Error)> TransitionNcrAsync(Guid projectId, Guid ncrId, NcrState toState, string? resolutionNotes = null)
    {
        try
        {
            var req = new GeneraPm.Services.SiteUserWorkflows.TransitionNcrRequest(toState, resolutionNotes);
    ...
    public async Task<(bool Ok, string? Error)> AssignNcrAsync(Guid projectId, Guid ncrId, Guid assignedToId)
    {
        try
        {
            var req = new GeneraPm.Services.SiteUserWorkflows.AssignNcrRequest(assignedToId);
