...
namespace GeneraPm.Services;

// ── Auth ──────────────────────────────────────────────────────────────────────
public class AuthService(
    GeneraPmDbContext db,
    IConfiguration cfg,
    InvitationService invitations,
    GeneraPm.Services.Auth.ILoginAttemptTracker loginTracker,
    AuditService audit)
{
    ...
    /// </summary>
    public async Task RevokeUserTokensAsync(Guid userId, GeneraPm.Services.Tenancy.ITenantContext tenant)
    {
    ...
    /// </summary>
    public async Task DeactivateUserAsync(Guid userId, GeneraPm.Services.Tenancy.ITenantContext tenant)
    {
        var query = tenant.IsSuperAdmin
            ? db.Users.IgnoreQueryFilters()
            : db.Users.AsQueryable();
    ...
    private string GenerateAccess(User user)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(GeneraPm.Services.Tenancy.HttpTenantContext.OrganisationClaimType, user.OrganisationId.ToString()),
        };
        if (user.GlobalRole is { } role)
            claims.Add(new Claim(GeneraPm.Services.Tenancy.HttpTenantContext.GlobalRoleClaimType, role.ToString()));
...
public class InvitationService(GeneraPmDbContext db, AuditService audit)
{
...
// ── Projects ──────────────────────────────────────────────────────────────────
public class ProjectsService(GeneraPmDbContext db, AuditService audit, GeneraPm.Services.Tenancy.ITenantContext tenant)
{
    ...
    public async Task<Project> CreateAsync(CreateProjectRequest req, Guid userId, string? ip, string? ua)
    {
        ...
        // (see GeneraPmDbContext query filter). Non-SuperAdmin callers are locked to
...
public class CostService(GeneraPmDbContext db, AuditService audit)
{
...
public class PaymentCertificatesService(GeneraPmDbContext db, AuditService audit)
{
...
public class VariationsService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
...
public class RisksService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
    ...
    /// </summary>
    public async Task<List<GeneraPm.Core.RiskMatrixCell>> GetMatrixAsync(
        Guid projectId, CancellationToken ct = default)
    {
        _ = await db.Projects.FirstOrDefaultAsync(p => p.Id == projectId, ct)
            ?? throw new NotFoundException("Project");
        var live = await db.Risks
            ...
            .ToListAsync(ct);
        return GeneraPm.Core.RiskMatrix.Build(live);
    }
    ...
    /// </summary>
    public async Task<GeneraPm.Core.MonteCarloResult> RunMonteCarloAsync(
        Guid projectId, int iterations, int? seed = null, CancellationToken ct = default)
    {
        ...
        var inputs = risks.Select(r => new GeneraPm.Core.MonteCarloInput
        {
        ...
        return GeneraPm.Core.MonteCarlo.Simulate(inputs, iterations, seed ?? Random.Shared.Next());
    }
...
// PR #33 pattern.
public class StakeholdersService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
    ...
    /// </summary>
    public async Task<List<GeneraPm.Core.StakeholderMatrixCell>> GetMatrixAsync(
        Guid projectId, CancellationToken ct = default)
    {
        _ = await db.Projects.FirstOrDefaultAsync(p => p.Id == projectId, ct)
            ?? throw new NotFoundException("Project");
        var live = await db.Stakeholders
            .Where(s => s.ProjectId == projectId && s.IsActive)
            .ToListAsync(ct);
        return GeneraPm.Core.StakeholderMatrix.Build(live);
    }
...
/// </summary>
public class CommunicationsService(GeneraPmDbContext db, AuditService audit)
{
...
/// </summary>
public class ScheduleService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
        ...
        string? ip = null, string? ua = null, CancellationToken ct = default)
    {
        ...
        var cycle = GeneraPm.Core.DependencyGraph.DetectCycle(allIds, edges);
        if (cycle.HasCycle)
            throw new ConflictException(
    ...
    /// parser handles the XML; this method maps parsed UIDs → Genera PM
        ...
        string? ip = null, string? ua = null, CancellationToken ct = default)
    {
        ...
        GeneraPm.Core.MsProjectXml.ImportResult parsed;
        try { parsed = GeneraPm.Core.MsProjectXml.Parse(xmlStream); }
...
/// </summary>
public class LpsService(GeneraPmDbContext db, AuditService audit)
{
    // ── Lookahead ───────────────────────────────────────────────────
...
/// <see cref="GeneraPm.Core.ChangeWorkflow"/> drives every transition;
...
/// </summary>
public class ChangeRequestService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
...
/// </summary>
public class ProcurementStrategyService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
...
/// in <see cref="GeneraPm.Core.TenderPackageWorkflow"/>; this
/// service wraps with persistence + audit-twin emission. Award
/// (T-S6-06) extends this service with the atomic Award→Contract
/// spawn.
/// </summary>
public class TenderPackagesService(GeneraPmDbContext db, AuditService audit)
{
    ...
    {
        if (!GeneraPm.Core.TenderPackageWorkflow.IsValidTransition(from, to))
            throw new ConflictException(
                $"Cannot transition TenderPackage from {from} to {to}");
        if (!GeneraPm.Core.TenderPackageWorkflow.CanTransition(from, to, role))
            throw new ForbiddenException(
                $"Role {role} cannot perform the {from} → {to} transition");
    }
...
/// </summary>
public class TendersService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
...
/// during Draft set-up.
/// </summary>
public class EvaluationService(GeneraPmDbContext db, AuditService audit)
{
    // ── Criteria ────────────────────────────────────────────────────
        ...
        Guid projectId, Guid tenderPackageId, CancellationToken ct = default)
    {
        ...
        var coreInput = criteria.Select(c =>
            new GeneraPm.Core.EvaluationMatrix.CriterionInput(c.Id, c.Weight)).ToList();
        var coreScores = scores.Select(s =>
            new GeneraPm.Core.EvaluationMatrix.ScoreInput(s.TenderId, s.CriterionId, s.Score)).ToList();
        var matrixResult = GeneraPm.Core.EvaluationMatrix.Compute(
...
/// </summary>
public class EarlyWarningsService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
...
/// in <see cref="GeneraPm.Core.CompensationEventWorkflow"/>; this
...
/// </summary>
public class CompensationEventsService(GeneraPmDbContext db, AuditService audit)
{
    /// <summary>
    ...
    {
        if (!GeneraPm.Core.CompensationEventWorkflow.IsValidTransition(from, to))
            throw new ConflictException(
                $"Cannot transition CompensationEvent from {from} to {to}");
        if (!GeneraPm.Core.CompensationEventWorkflow.CanTransition(from, to, role))
            throw new ForbiddenException(
                $"Role {role} cannot perform the {from} → {to} transition");
    }
}
...
/// </summary>
public class DashboardsService(GeneraPmDbContext db)
{
...
public class ReportingService(GeneraPmDbContext db)
{
...
public class MidpService(GeneraPmDbContext db, AuditService audit)
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (req.DocTypeFilter is { } t && !GeneraPm.Core.Iso19650Codes.TypeCodeSet.Contains(t))
            throw new ValidationException(new List<string>
        ...
        CancellationToken ct = default)
    {
        ...
        if (req.DocTypeFilter is not null && req.DocTypeFilter != x.DocTypeFilter)
        {
            ...
            if (!GeneraPm.Core.Iso19650Codes.TypeCodeSet.Contains(t))
                throw new ValidationException(new List<string>
                { $"DocTypeFilter '{t}' is not in the ISO 19650-2 Annex A type whitelist" });
...
public class TidpService(GeneraPmDbContext db, AuditService audit)
{
...
// // GDPR ref: Art. 30.
public class RopaService(GeneraPmDbContext db, AuditService audit, GeneraPm.Services.Tenancy.ITenantContext tenant)
{
    private Guid OrgId =>
        tenant.OrganisationId ?? throw new ForbiddenException("No tenant context");
...
// Core/DpiaWorkflow.
// // GDPR ref: Art. 35.
public class DpiaService(GeneraPmDbContext db, AuditService audit)
{
...
// // GDPR ref: Art. 12, 15.
public class SarService(GeneraPmDbContext db, AuditService audit, GeneraPm.Services.Tenancy.ITenantContext tenant)
{
    private Guid OrgId =>
        tenant.OrganisationId ?? throw new ForbiddenException("No tenant context");
...
// // GDPR ref: Art. 33, 34.
public class DataBreachService(GeneraPmDbContext db, AuditService audit, GeneraPm.Services.Tenancy.ITenantContext tenant)
{
    private Guid OrgId =>
        tenant.OrganisationId ?? throw new ForbiddenException("No tenant context");
...
// B-078.
// // GDPR ref: Art. 5(1)(e).
public class RetentionScheduleService(GeneraPmDbContext db, AuditService audit, GeneraPm.Services.Tenancy.ITenantContext tenant)
{
    private Guid OrgId =>
        tenant.OrganisationId ?? throw new ForbiddenException("No tenant context");
...
public class ImprovementRegisterService(GeneraPmDbContext db, AuditService audit)
{
...
public class LessonsLearnedService(GeneraPmDbContext db, AuditService audit, GeneraPm.Services.Tenancy.ITenantContext tenant)
{
    private Guid OrgId =>
        tenant.OrganisationId ?? throw new ForbiddenException("No tenant context");
...
// text type drift).
public class OpportunityToImproveService(GeneraPmDbContext db, AuditService audit)
{
...
// bullet (Genera PM quality record half). v1.0 manual workflow;
// Genera bidirectional sync deferred to v1.1 / B-089.
public class InspectionActivityService(GeneraPmDbContext db, AuditService audit)
{
...
// GeneraPm.Services.Alerts.ThresholdEvaluator (pure logic) and
// ThresholdEvaluatorHostedService (background loop).
public class AlertRuleService(GeneraPmDbContext db, AuditService audit)
{
...
public class GatewayPackageService(GeneraPmDbContext db, AuditService audit)
{
...
public class MorService(GeneraPmDbContext db, AuditService audit)
{
...
public class SafetyCaseService(GeneraPmDbContext db)
{
...
public class CustomReportDefinitionsService(GeneraPmDbContext db, AuditService audit)
{
...
public class CdeService(GeneraPmDbContext db, AuditService audit)
{
...
public class DocumentsService(
    GeneraPmDbContext db,
    AuditService audit,
    GeneraPm.Services.Iso19650.Iso19650FilenameValidator iso19650)
{
    ...
    public async Task<Document> CreateAsync(Guid projectId, CreateDocumentRequest req, Guid userId, string? ip, string? ua)
    {
        ...
        var blocking = validation.Checks
            .Where(c => !c.Passed
                     && (c.Id == GeneraPm.Services.Iso19650.Iso19650CheckId.Structure
                      || c.Id == GeneraPm.Services.Iso19650.Iso19650CheckId.FieldValidity
                      || c.Id == GeneraPm.Services.Iso19650.Iso19650CheckId.Numbering))
...
// ── RFIs ──────────────────────────────────────────────────────────────────────
public class RfiService(GeneraPmDbContext db, AuditService audit)
{
...
// ── Actions ───────────────────────────────────────────────────────────────────
public class ActionsService(GeneraPmDbContext db, AuditService audit)
{
