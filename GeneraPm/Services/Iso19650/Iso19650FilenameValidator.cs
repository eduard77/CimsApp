...
namespace GeneraPm.Services.Iso19650;
...
public sealed class Iso19650FilenameValidator
{
    ...
    public Iso19650FilenameValidationResult Validate(string filename)
    {
        ...
        var typeOk  = GeneraPm.Core.Iso19650Codes.TypeCodeSet.Contains(type);
        var roleOk  = GeneraPm.Core.Iso19650Codes.RoleCodeSet.Contains(role);
        ...
        // Level, etc.
        var volOk   = GeneraPm.Core.Iso19650Codes.VolumeCodeSet.Contains(volume);
        var levelOk = GeneraPm.Core.Iso19650Codes.LevelCodeSet.Contains(level);
        ...
        var suitOk = GeneraPm.Core.Iso19650Codes.SuitabilityCodeSet.Contains(suitability);
        checks.Add(new Iso19650CheckOutcome(
    ...
    {
        if (GeneraPm.Core.Iso19650Codes.TypeToUniclass.TryGetValue(type, out var uniclass))
        {
    ...
    private static Iso19650CheckOutcome CheckUniclassHierarchy(string type)
    {
        ...
        if (GeneraPm.Core.Iso19650Codes.DeprecatedUniclassCodes.Contains(uniclass))
        {
            return new Iso19650CheckOutcome(
                Iso19650CheckId.UniclassHierarchy,
                "Uniclass hierarchy validity",
    ...
    {
        var hasUniclass = GeneraPm.Core.Iso19650Codes.TypeToUniclass.TryGetValue(type, out var uniclass);
        var hasIfc = GeneraPm.Core.Iso19650Codes.TypeToIfc.TryGetValue(type, out var ifc);
        ...
        if (!GeneraPm.Core.Iso19650Codes.UniclassTableToIfcPrefixes.TryGetValue(
