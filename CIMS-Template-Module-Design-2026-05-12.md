# CIMS Template Module — Design Document
...
This document captures the design decisions made in the 2026-05-12 session for what is now established as **the most important module in CIMS: the Template Module**.

The Template Module is the structured-data spine of CIMS. Every project artefact a UK PM Consultancy produces (Business Case, Charter, Risk Register, Stakeholder Register, Communications Plan, all subsidiary management plans, NEC4 contract artefacts, IPA Gateway reviews, RIBA stage outputs, CDM artefacts, ISO 19650 information delivery plans) is expressed as a Template instance within this module. The module is what makes CIMS "complete PM software" rather than a document register.
...
1. **Build CIMS into a complete PM software platform aligned with UK construction PM practice.** This is the primary commercial deliverable. The Template Module is the architectural enabler.
...
This reframes CIMS. It is not a document management system that happens to have structured templates. **It is a structured data platform that happens to produce documents.** Every CIMS feature downstream (risk analysis, schedule optimisation, change control, earned value, dashboards, AI insights) is a data consumer of the templates. The fidelity of every analysis depends on the discipline enforced at template creation.
...
3. **Differentiation from competitors.** Procore, Aconex, Asite all accept arbitrary uploads. CIMS does not. This is a deliberate product positioning.
...
- Forces breadth across PMBOK and UK frameworks (which is what CIMS needs).
...
Selected over Client-side or Main Contractor-side. Most artefact-heavy of the three positions. Closest to Eduard's stated career trajectory (consultancy build, public sector framework targets). Closest to CIMS's eventual buyer audience.
...
- Code organised as `CimsApp.Core` (region-agnostic) + `CimsApp.Regions.UK` (UK pack) + future `Regions.US`, `Regions.AU` packs.
...
| 3 | ISO 19650 number sequencing | **Per-Type sequential**, scoped to Project + Originator + Type. Confirms existing CIMS behaviour. |
...
The retirement of the file-upload path is a clean breaking change: future versions of CIMS only create template-backed documents.
...
The flow: WIP → Shared → Published → Archived. Already in CIMS for unstructured documents.
...
| Risk | Description |
|---|---|
| Schedule Optimisation deprioritised | Building the Template Module makes CIMS the lead product by consumed-time, even if Schedule Optimisation remains the *stated* lead. Strategic decision should be made consciously, not by drift. |
| Existing CIMS data invalidated by upload-path removal | Today's tested document on Smoke Test 5 is created via the upload path. New model retires this path. Decision needed: migration, deletion, or accept-as-legacy. Recommend accept-as-legacy. |
...
| TKT-010 | Decision needed: Schedule module ownership (CIMS vs Schedule Optimisation Platform vs shared core) | Medium | Open — strategic |
...
- Consider scheduling a strategic review of Schedule Optimisation Platform vs CIMS priority (TKT-010-adjacent). Not a design question; a portfolio question.
