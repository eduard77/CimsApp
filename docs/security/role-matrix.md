...
| GET    | `/api/v1/projects/{projectId}/custom-reports` | authenticated | membership | T-S7-05. Lists active custom report definitions for the project, ordered by Name. |
| GET    | `/api/v1/projects/{projectId}/custom-reports/{definitionId}` | authenticated | membership | T-S7-05. Cross-tenant 404 via the query filter. |
| POST   | `/api/v1/projects/{projectId}/custom-reports` | authenticated | `TaskTeamMember+` | T-S7-05. Body `CreateCustomReportDefinitionRequest(name, entityType, filterJson?, columnsJson?)`. EntityType ∈ {Risk, ActionItem, Rfi, Variation, ChangeRequest}. FilterJson + ColumnsJson validated at write time against the per-entity field allow-list (`Core/CustomReportRunner`). Name unique per project among IsActive rows. Audit: `custom_report.created`. |
| PUT    | `/api/v1/projects/{projectId}/custom-reports/{definitionId}` | authenticated | `TaskTeamMember+` | T-S7-05. Body `UpdateCustomReportDefinitionRequest` — partial update of Name / FilterJson / ColumnsJson. EntityType is immutable post-create. No-op rejected with 409. Audit: `custom_report.updated` with `{ changedFields }`. |
| DELETE | `/api/v1/projects/{projectId}/custom-reports/{definitionId}` | authenticated | `TaskTeamMember+` | T-S7-05. Soft delete via IsActive = false. Audit: `custom_report.deleted`. |
| POST   | `/api/v1/projects/{projectId}/custom-reports/{definitionId}/run` | authenticated | membership | T-S7-05. Executes the saved query: loads project's rows of the saved EntityType, applies the equality filter in-memory, projects the requested columns. Returns `CustomReportRunResultDto(definitionId, name, entityType, rowCount, columns, rows)`. Read-only — no audit. v1.0 limitations: cross-entity joins → B-056; richer operators → B-060; export formats → B-058; scheduled runs → B-057. |
