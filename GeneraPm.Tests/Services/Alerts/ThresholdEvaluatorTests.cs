...
using GeneraPm.Services.Alerts;
...
namespace GeneraPm.Tests.Services.Alerts;
...
public class ThresholdEvaluatorTests
{
    ...
    private static GeneraPm.Models.Risk NewRisk(Guid projectId, RiskStatus status) => new()
    {
        ProjectId = projectId,
        Title = "Risk",
        Description = "x",
    ...
    private static (GeneraPmDbContext db, Guid projectId, Guid orgId) BuildSeededDb()
    {
        ...
        return (new GeneraPmDbContext(options, tenant), projectId, orgId);
    }
}
