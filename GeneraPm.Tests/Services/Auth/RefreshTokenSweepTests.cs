...
{
    private static (DbContextOptions<GeneraPmDbContext> options, IConfiguration cfg,
        ...
        IConfiguration cfg, StubTenantContext tenant)
    {
        ...
        return new AuthService(db, cfg, new InvitationService(db, new AuditService(db)), tracker, new AuditService(db));
    }

    private static (Guid activeId, Guid alreadyRevokedId, Guid expiredId)
        SeedRefreshTokensFor(DbContextOptions<GeneraPmDbContext> options,
    ...
    private static async Task<DateTime?> ReadRevokedAtAsync(
        DbContextOptions<GeneraPmDbContext> options, StubTenantContext tenant, Guid tokenId)
    {
