...
public class AuthServiceInputValidationTests
{
    private static AuthService BuildService()
    {
        ...
        var db = new GeneraPmDbContext(options, tenant);
        ...
        return new AuthService(db, cfg, new InvitationService(db, new AuditService(db)), NewTracker(), new AuditService(db));
    }
    ...
    private static (DbContextOptions<GeneraPmDbContext> options, IConfiguration cfg, Guid orgA, Guid orgB, Guid userInA, Guid userInB)
    ...
    public async Task RevokeOwnTokens_sets_cutoff_to_UtcNow()
    {
        ...
        {
            var svc = new AuthService(db, cfg, new InvitationService(db, new AuditService(db)), NewTracker(), new AuditService(db));
            await svc.RevokeOwnTokensAsync(userInA);
        }
        var after = DateTime.UtcNow;

        using var verify = new GeneraPmDbContext(options, tenant);
    ...
    public async Task RevokeUserTokens_admin_OrgAdmin_can_target_own_org_user()
    {
        ...
        using (var db = new GeneraPmDbContext(options, orgAdmin))
        {
            var svc = new AuthService(db, cfg, new InvitationService(db, new AuditService(db)), NewTracker(), new AuditService(db));
            await svc.RevokeUserTokensAsync(userInA, orgAdmin);
        }

        using var verify = new GeneraPmDbContext(options, orgAdmin);
    ...
    public async Task RevokeUserTokens_admin_SuperAdmin_can_target_any_org_user()
    {
        ...
        using (var db = new GeneraPmDbContext(options, superAdmin))
        {
            var svc = new AuthService(db, cfg, new InvitationService(db, new AuditService(db)), NewTracker(), new AuditService(db));
            await svc.RevokeUserTokensAsync(userInB, superAdmin);
        }

        using var verify = new GeneraPmDbContext(options, superAdmin);
    ...
    public async Task RevokeUserTokens_admin_emits_auth_user_admin_revoke_audit()
    {
        ...
        using (var seed = new GeneraPmDbContext(options, orgAdmin))
        {
