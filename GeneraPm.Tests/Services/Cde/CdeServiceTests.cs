...
{
    private static (DbContextOptions<GeneraPmDbContext> options, StubTenantContext tenant,
    ...
    public async Task ListContainers_returns_active_containers_for_project()
    {
        ...
        using var db2 = new GeneraPmDbContext(options, tenant);
    ...
    public async Task ListContainers_excludes_inactive_containers()
    {
        ...
        using (var seedDb = new GeneraPmDbContext(options, tenant))
        {
        ...
        // honours `IsActive`).
        using (var deactivateDb = new GeneraPmDbContext(options, tenant))
        {
    ...
    public async Task ListContainers_returns_empty_for_cross_tenant_project()
    {
        ...
        var optionsB = new DbContextOptionsBuilder<GeneraPmDbContext>()
            .UseInMemoryDatabase(dbName)
            .AddInterceptors(new AuditInterceptor(tenantB, httpAccessor: null))
            .Options;
        using (var seed = new GeneraPmDbContext(optionsB, tenantB))
        {
        ...
        using (var db = new GeneraPmDbContext(optionsB, tenantB))
        {
            var svc = new CdeService(db, new AuditService(db));
        ...
        var optionsA = new DbContextOptionsBuilder<GeneraPmDbContext>()
            .UseInMemoryDatabase(dbName).Options;
        using var dbA = new GeneraPmDbContext(optionsA, tenantA);
