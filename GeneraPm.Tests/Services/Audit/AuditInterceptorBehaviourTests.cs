...
namespace GeneraPm.Tests.Services.Audit;
...
{
    private static (DbContextOptions<GeneraPmDbContext> options, StubTenantContext tenant, Guid orgId, Guid userId)
        BuildFixture()
    {
    ...
    public async Task Direct_AuditLog_insert_does_not_recurse()
    {
        ...
        using var verify2 = new GeneraPmDbContext(options, tenant);
    ...
    public async Task Anonymous_tenant_context_skips_audit_capture()
    {
        ...
        using (var seed = new GeneraPmDbContext(options, anonTenant))
        {
            seed.Organisations.Add(new Organisation { Id = orgId, Name = "Anon Org", Code = "AO" });
            await seed.SaveChangesAsync();
        }
        using (var db = new GeneraPmDbContext(options, anonTenant))
        {
        ...
        using var verify = new GeneraPmDbContext(options, anonTenant);
