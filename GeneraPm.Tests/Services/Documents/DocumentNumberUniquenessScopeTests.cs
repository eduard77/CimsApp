...
namespace GeneraPm.Tests.Services.Documents;
...
{
    private static (DbContextOptions<GeneraPmDbContext> options, Guid orgA, Guid orgB,
    ...
    public async Task Two_tenants_with_same_project_code_can_each_create_DOC_0001()
    {
        ...
        using (var dbA = new GeneraPmDbContext(options, tenantA))
        {
            var svc = new DocumentsService(dbA, new AuditService(dbA), new GeneraPm.Services.Iso19650.Iso19650FilenameValidator());
        ...
        using (var dbB = new GeneraPmDbContext(options, tenantB))
        {
            var svc = new DocumentsService(dbB, new AuditService(dbB), new GeneraPm.Services.Iso19650.Iso19650FilenameValidator());
        ...
        using var verify = new GeneraPmDbContext(options, seedTenant);
    ...
    public async Task Same_tenant_duplicate_DocumentNumber_still_throws_ConflictException()
    {
        ...
        using var db = new GeneraPmDbContext(options, tenantA);
        var svc = new DocumentsService(db, new AuditService(db), new GeneraPm.Services.Iso19650.Iso19650FilenameValidator());
        await svc.CreateAsync(projectInA, NewRequest(), userA, null, null);
