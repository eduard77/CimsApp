...
using GeneraPm.Services.Iso19650;
using GeneraPm.Tests.TestDoubles;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Diagnostics;
using Xunit;

namespace GeneraPm.Tests.Services.Bsa2022;
...
public class MorAndSafetyCaseTests
{
    ...
    public async Task MorService_CreateAsync_assigns_sequential_MOR_numbers()
    {
        ...
        var svc = new MorService(new GeneraPmDbContext(options, tenant),
            new AuditService(new GeneraPmDbContext(options, tenant)));
    ...
    public async Task SafetyCaseService_GenerateAsync_aggregates_BSA_2022_state()
    {
        ...
        {
            db.Risks.AddRange(
                new GeneraPm.Models.Risk { ProjectId = projectId, Title = "R1", Probability = 3, Impact = 3, Score = 9, Status = RiskStatus.Active },
                new GeneraPm.Models.Risk { ProjectId = projectId, Title = "R2", Probability = 2, Impact = 2, Score = 4, Status = RiskStatus.Identified });
            db.Rfis.Add(new Rfi
            {
