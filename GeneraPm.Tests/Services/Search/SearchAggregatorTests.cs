...
using GeneraPm.Services.Search;
using GeneraPm.Tests.TestDoubles;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Diagnostics;
using Xunit;

namespace GeneraPm.Tests.Services.Search;
...
public class SearchAggregatorTests
{
    ...
    public async Task Tenant_isolation_excludes_other_orgs_data()
    {
        ...
        using (var seed = new GeneraPmDbContext(options, tenant))
        {
            ...
            seed.Risks.Add(new GeneraPm.Models.Risk { ProjectId = projA, Title = "Foundation risk A", Description = "x", Probability = 3, Impact = 3 });
            seed.Risks.Add(new GeneraPm.Models.Risk { ProjectId = projB, Title = "Foundation risk B", Description = "x", Probability = 3, Impact = 3 });
            seed.SaveChanges();
        }

        var db = new GeneraPmDbContext(options, tenant); // tenant A
        var svc = BuildAggregator(db);
    ...
    private static (GeneraPmDbContext db, Guid projectId, Guid userId) BuildFixture()
    {
        ...
        return (new GeneraPmDbContext(options, tenant), projectId, userId);
    }

    private static SearchAggregatorService BuildAggregator(GeneraPmDbContext db)
    ...
    private static void SeedDoc(GeneraPmDbContext db, Guid projectId, string number, string title, string desc)
    {
    ...
    private static void SeedRfi(GeneraPmDbContext db, Guid projectId, Guid userId, string number, string subject, string desc)
    {
        db.Rfis.Add(new Rfi
        {
            ProjectId = projectId,
    ...
    private static void SeedAction(GeneraPmDbContext db, Guid projectId, Guid userId, string title, string desc)
    {
    ...
    private static void SeedRisk(GeneraPmDbContext db, Guid projectId, string title, string desc)
    {
        db.Risks.Add(new GeneraPm.Models.Risk
        {
            ProjectId = projectId,
            Title = title, Description = desc,
    ...
    private static void SeedChangeRequest(GeneraPmDbContext db, Guid projectId, Guid userId, string number, string title, string desc)
    {
